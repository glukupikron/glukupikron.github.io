...
## Owari 종료 및 플레이 기록 이미지 계획

### 현재 구현 상태

- HTML에 왼쪽 상단 고정 버튼 `.owariButton`이 있다.
- 버튼에 표시되는 문구는 `Done?`이며 `onclick="owari()"`를 호출한다.
- `.owariButton`의 기본 디자인 CSS는 작성되어 있다.
- `owari()` 함수, 종료 모달, 화면 캡처와 다운로드 기능은 아직 구현하지 않았다.

### 우선 구현할 로컬 다운로드 버전

현재 단계에서는 임시 서버나 QR 전송을 붙이지 않고 다음 흐름까지만 구현한다.

1. 플레이어가 Owari 버튼을 누른다.
2. 종료 확인 모달을 표시한다.
3. 모달에는 Done과 Cancel 버튼을 둔다.
4. Done을 누르면 지금까지 누적된 `.eventArea` 전체를 이미지로 캡처한다.
5. 캡처 결과를 PNG 파일로 현재 기기에 다운로드한다.
6. 다운로드가 끝나면 모달을 닫는다.

- 전체 `body`가 아니라 `.eventArea`를 캡처 대상으로 사용한다. 시작 버튼, Owari 버튼과 고정 UI를 결과에서 제외하기 위해서다.
- 캡처에는 `html2canvas` 같은 브라우저용 라이브러리를 사용할 예정이다.
- 긴 플레이 기록은 결과 이미지가 매우 커질 수 있으므로 실제 테스트 후 축소 비율이나 최대 길이를 검토한다.
- 외부 CDN, 이미지 CORS, GIF와 복잡한 CSS가 캡처 결과에 미치는 영향은 배포 환경에서 확인한다.

구현할 함수는 역할을 분리한다.

```js
async function captureJourney() {
    // .eventArea를 캡처하고 이미지 데이터를 반환
}

function downloadJourney(image) {
    // 캡처 이미지를 PNG로 현재 기기에 다운로드
}

async function finishJourney() {
    const image = await captureJourney();
    downloadJourney(image);
}
```

### 나중에 추가할 전시용 QR 버전

전시에서는 같은 캡처 이미지 하나를 로컬에 보관하면서 관람객의 휴대폰에도 전달하는 방향으로 확장한다.
...
→ Done 클릭
→ .eventArea 전체 캡처
    ├→ 전시 기기에 PNG 다운로드 및 보관
    └→ 임시 서버에 동일한 PNG 업로드
          → 이미지 임시 주소 생성
          → 주소를 QR 코드로 표시
          → 관람객이 휴대폰으로 이미지 확인 및 저장
```

- QR 코드에 이미지 자체를 넣는 것이 아니라 업로드된 이미지의 임시 URL을 넣는다.
- GitHub Pages는 정적 호스팅이므로 업로드를 위해 별도 서버 또는 외부 스토리지가 필요하다.
- 서버 업로드, 임시 URL 생성, QR 표시는 전시 준비가 구체화될 때 추가한다.
- 관람객 이미지의 보관 기간과 자동 삭제 정책도 전시 전에 정한다.
- 권장 전시 순서는 QR을 먼저 보여주는 방식이 아니라 `Done → 캡처 및 업로드 → QR 표시`다. 이 방식이 기기 간 실시간 대기 상태를 연결하지 않아도 되어 더 단순하고 안정적이다.
...
async function finishJourney() {
    ...
    // 전시 버전에서 나중에 추가
    // const imageUrl = await uploadJourney(image);
    // showQr(imageUrl);
}
```
